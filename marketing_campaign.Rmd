...
control_ctr <- campaigns %>% 
  dplyr::filter(campaign_name == 'Control Campaign') %>% 
  dplyr::select(ctr)


shapiro.test(control_ctr$ctr)

test_ctr <- campaigns %>% 
  dplyr::filter(campaign_name == 'Test Campaign') %>% 
  dplyr::select(ctr)


shapiro.test(test_ctr$ctr)
...
Analyzing the distributions of the two samples in the density plots, along with the mean and median values of the CTR of the two campaigns, there is a tendency to classify the test campaign as more successful than the test campaign.
...
wilcox.test(control_ctr$ctr, test_ctr$ctr,
            alternative = 'less',
            var.equal = FALSE,
            paired = FALSE)
